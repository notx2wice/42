...
/*   Updated: 2021/01/14 20:34:20 by ukim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
)
{
	...
	if ((int)ft_strlen(line) >= i && check_player_in_map(line[i]))
	{
		...
		window->player->pos.x = (double)idx - (BONUS_TEX_CNT + 3) + 0.5;
		pos_cnt++;
		window->cub->worldmap[idx - (BONUS_TEX_CNT + 3)][i] = '0';
	}
	else
		window->cub->worldmap[idx - (BONUS_TEX_CNT + 3)][i] = 'X';
	return (pos_cnt);
}
...
)
{
	...
	while (i < window->cub->map_col)
	{
		...
		{
			window->cub->worldmap[idx - (BONUS_TEX_CNT + 3)][i] = line[i];
			if (line[i] == '2')
